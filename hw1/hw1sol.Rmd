...
1. Display the first few lines of `NYCHVS_1991.csv`.
    ...
    **Solution：**Here is the command used to display the first few lines of `NYCHVS_1991.csv`.
    ```{bash}
    cd /home/m280data/NYCHVS/
    head NYCHVS_1991.csv
    ```
    
2. Display number of lines in each `csv` file.
   
    **Solution：**Here is the command used to display number of lines in each `csv` file.
    ```{bash}
...
3. Display the 3 files that have the least number of lines
    ...
    **Solution：**Here is the command used to display the 3 files that have the least number of lines.
    ```{bash}
    cd /home/m280data/NYCHVS/
    wc -l  *csv | sort | head -3
    ```
...
4. What's the output of following bash script?
    ...
    **Solution：**The output display the path that we use to open all csv files in NYCHVS.
    
5. What unique values does the second variable `borough` take in `NYCHVS_1991.csv`? Tabulate how many times each value appears.

    **Solution：**The second variable `borough` take 1-5 as unique values. And the time each value appears is displayed as follows.
    ```{bash}
    cd /home/m280data/NYCHVS/
    awk -F, '{ if (NR > 2) print $2 }' NYCHVS_1991.csv | sort | uniq -c
    ```
...
Do **not** put this text file `pride_and_prejudice.txt` in Git. Using a `for` loop, how would you tabulate the number of times each of the four characters is mentioned?
    ...
    **Solution：**Here are the commands I used to tabulate the number of times each of the four characters is mentioned. And from the results we can know that Elizabeth was the most mentioned character. 
    ```{bash}
    filename="pride_and_prejudice.txt"
    for name in Elizabeth Jane Lydia Darcy  
       do echo "${name} - `grep "${name}" ${filename} | wc -l`,"
    done
    ```
...
2. What's the difference between the following two commands?
    ...
    **Solution:** After executing each command twice in a row and then examining the output files. I find that there are two 'hello, world' in test2, while only one in test1.
    The reason for that is command ">" directs output from one command to a file, while command ">>" appends output from one command to a file.
...
Explain the output. Explain the meaning of `"$1"`, `"$2"`, and `"$3"` in this shell script. Why do we need the first line of the shell script?
    ...
    **Solution:**Here are the commands I used to make the file executable by the the owner. 
    ```{bash eval=FALSE}
     vi middle.sh
     chmod 764 middle.sh 
    ```
    Then we run this command and get the output as follows.
    ```{bash}
    ./middle.sh pride_and_prejudice.txt 20 5
    ``` 
    
    The output is the 16-20 lines in pride_and_prejudice.txt.
    
    The `"$1"`,`"$2"`, and `"$3"` are the first, second and third column of the commend. In this question, they are `pride_and_prejudice.txt`, `20` and `5` seperately.
    
    Thus, the whole command in middle.sh means we print the first  `"$2"` lines of the file `"$1"`, then print the last `"$3"` lines of the output.
    
    We need the first line of the shell script because if the file that this script lives in is executable, the hash-bang (#!) tells the operating system what interpreter to use to run the script. In this case it's /bin/sh, for example.
...
for both methods.
    ...
    **Solution:** Here is my `runSimR` code:
    ```{bash }
    cat runSim.R
    ```

    Let us test `runSiM.R` with ` seed = 1234 n = 100 dist = t1 rep = 10`:
    ```{bash }
    Rscript runSim.R seed=1234 n=100 dist=\"t1\" rep=10
    ```
...
3. Write an R script to collect simulation results from output files and print average MSEs in a table of format
    ...
    **Solution:** Here is my code for collecting data from output files and printing them into a table:
    ```{bash}
    cat Collect.R
    ```

    Here is the table that I got.
    ```{bash}
    Rscript Collect.R 
    ``` 
